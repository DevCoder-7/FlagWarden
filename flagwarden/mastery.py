...
def apply_solve_mastery(
    db: Session, user_id: int, skills: list[str], difficulty: str, hints_used: int
) -> None:
    gain = mastery_gain(difficulty, hints_used)
    for skill in skills:
        row = db.scalar(
            select(SkillMastery).where(SkillMastery.user_id == user_id, SkillMastery.skill == skill)
        )
        if row is None:
